...
- `docs/skill-logs/defect_repair_${CATEGORY}_${DATE}.md` に開始ログを記載
  ...
  ### 不具合タイトル
  [ProcDataSync] InvalidCastException when processing DATA_SYNC telegram

  ### 症状
  - エラー内容: InvalidCastException in DataRecord.GetXxx()
  - 発生タイミング: DATA_SYNC telegram 受信時
  - 影響: ProcDataSync プロセスが異常終了
  ...
  ### 再現ステップ
  1. 特定形式の DS telegram を送信
  2. ProcDataSync で処理開始
  3. エラー出力、プロセス停止
...
### レビュー内容
**変更ファイル**: ProcDataSync.cs, LibDataCommon.cs
...
| ファイル | 変更内容 | 行数 |
|---------|---------|------|
| ProcDataSync.cs | NULL判定ロジック追加 | +5 |
| LibDataCommon.cs | DataRecordExtensions 活用 | +8 |
| ProcDataSyncTest.cs | テストケース追加 | +30 |
```
