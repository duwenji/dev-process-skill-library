...
具体的な記入例（架空の不具合対応シナリオを通した14段階の記入イメージ）は [example.md](./example.md) を参照してください。本Runbookは手順・判定基準のみを記載します。
...
1. 不具合タイトルを記述
   ...
   - 例: `[ProcDataSync] InvalidCastException when processing xxx telegram`
...
**記録**:
- `docs/skill-logs/defect_repair_${CATEGORY}_${DATE}.md` に開始ログを記載
  ```
  ## 段階3: 開発者が不具合内容を入力（YYYY-MM-DD HH:MM:SS +09:00）

  ### 不具合タイトル
  [Module] Symptom（エラー内容、機能名）
  ...
  **承認ステータス**: 未承認（段階4開始前、確認待機）
  ```
  記入例は [example.md](./example.md) の「段階3」を参照
...
Mermaid サンプル（対応前/対応後を1つの図で比較する記法例）は [example.md](./example.md) の「段階5」を参照してください。
...
---
```
記入例は [example.md](./example.md) の「段階7」を参照
...
**記録**:
```
## 段階9: 開発者がソースコードをレビュー・承認（YYYY-MM-DD HH:MM:SS +09:00）
...
---
```
記入例は [example.md](./example.md) の「段階8-9」を参照
...
public void Test_ProcessDsTelegram(string input)
{
    // Arrange
    // Act
    // Assert
...
**記録**:
```
## 段階11: 開発者がチェック項目をレビュー・承認（YYYY-MM-DD HH:MM:SS +09:00）

### レビュー内容
**チェック項目数**: [件数]（正常系N, 境界値N, 異常系N）
...
**ダミー実装合意**:
- [TRK-xxx（対象）]: ダミー実装で対応（理由）

**承認ステータス**: ✓ 承認済

---
```
記入例は [example.md](./example.md) の「段階10-11」を参照
...
**テスト結果テンプレート**:
```
| TRK-001 | 正常系 | DS telegram 正常入力 | ✓ 合格 | 期待値と一致 |
...
**ダミー実装内容の明示**: 外部依存（DB接続等）をモック化した場合は、対象TRK、モック化内容、実施結果を記載する。記入例は [example.md](./example.md) の「TRK-007のダミー実装内容」を参照。
...
---
```
記入例は [example.md](./example.md) の「段階13」を参照
...
記入例（段階3〜14の記入イメージ）は [example.md](./example.md) を参照。
...
- **完全なテスト結果**: docs/skill-logs/defect_repair_${CATEGORY}_${DATE}.md 参照
...
docs/skill-logs/defect_repair_IPC_20260327.md
docs/skill-logs/defect_repair_Performance_20260327.md
```
...
**記録完了日時**: [YYYY-MM-DD HH:MM:SS +09:00]
**承認ステータス**: ✓ 全段階承認済
```
