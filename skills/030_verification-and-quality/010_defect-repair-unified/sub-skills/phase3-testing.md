...
- [ ] CSV（Excel確認用）と Markdown 正本に差分矛盾がないか
...
- [ ] ビルド・静的チェック成功か
- [ ] 実行ステータス別・実施方法別・分類別の集計結果が記録されているか
...
| TRK | 分類 | テスト項目 | 実施方法 | 実施手順 | 実行ステータス | 実行者 | 期待結果 | 備考 |
|-----|------|----------|--------|--------|--------------|--------|--------|------|
| TRK-001 | 正常系 | 全フィールド有効値で処理実行 | 自動（ユニットテスト） | `dotnet test --filter TRK-001` 実行後にログ確認 | 未実施 | AI | データ項目を正常に処理 | 既存テスト ProcessDataRecord_ValidInput() |
| TRK-002 | 境界値 | DISPLAY_NAME = NULL で処理実行 | 自動（新規テスト） | NULL入力モックで既定値適用を Assert | 未実施 | AI | 既定値「(未設定)」を適用、正常処理継続 | DataRecordExtensions.GetStringOrNull() の動作確認 |
| TRK-003 | 境界値 | RECORD_ID = NULL で処理実行 | 自動（新規テスト） | NULL入力モックで既定値適用を Assert | 未実施 | AI | 既定値「」を適用、正常処理継続 | 前後の処理で影響がないこと確認 |
| TRK-004 | 境界値 | CATEGORY_CODE = NULL で処理実行 | 自動（新規テスト） | NULL入力モックで既定値適用を Assert | 未実施 | AI | 既定値 0 を適用、正常処理継続 | 商域コード用の適切な既定値の確認 |
| TRK-005 | 境界値 | DISPLAY_NAME, RECORD_ID 両方 NULL | 自動（新規テスト） | 複合NULL入力で既定値同時適用を確認 | 未実施 | AI | 両既定値を適用、正常処理継続 | 複合NULL条件の確認 |
| TRK-006 | 異常系 | CATEGORY_CODE に文字列が入っている場合 | 自動（新規テスト） | 型不一致入力で例外型とログを検証 | 未実施 | AI | Exception ハンドリング、ログ出力 | GetInt32OrNull() で型チェック実施 |
| TRK-007 | 異常系 | DB接続エラー時の Exception ハンドリング | ダミー実装 | Mock DataRecord で SqlException を投出 | 未実施 | AI | エラーログ出力、上位への例外伝播 | Mock DataRecord で SqlException を投出 |
| TRK-008 | 統合 | 複数台の DS telegram 連続処理 | 手動/自動 | 連続送信後に件数・メモリ使用量・ログを照合 | 未実施 | 手動担当 | 各telegram が独立に処理される、メモリリークなし | ストレステスト的観点 |
| TRK-009 | 正常系 | 既存ユニットテスト群全実行（回帰避止） | 自動 | `dotnet test` 全件実行で失敗件数0を確認 | 未実施 | AI | 全テスト合格（既存テスト影響なし） | 回帰テスト |
...
1. 上記チェック項目表を CSV に変換（列順固定）
2. UTF-8 BOM で保存し Excel で確認
3. 段階11承認前に Markdown 正本と CSV の差分を突合
4. 段階12終了後に `実行ステータス` を両方更新

### 段階11で確認する集計（初期値）

| 集計軸 | 確認内容 |
|------|---------|
| 実行ステータス別 | 全項目が「未実施」で初期化されているか |
| 実施方法別 | 自動/手動/ダミー実装の割当が妥当か |
| 分類別 | 正常系/境界値/異常系/統合/回帰の網羅性 |
...
- ✓ CSV（Excel確認用）を作成し、列崩れなし
- ✓ Markdown 正本と CSV の TRK/実施方法/実施手順 が一致
- ✓ 3軸集計（実行ステータス別・実施方法別・分類別）の初期値を確認
...
| 項目 | TRK | 実施方法 | 実行ステータス | 結果 | 実行者 | 備考 |
|------|-----|--------|--------------|------|--------|------|
| 正常系 | TRK-001 | 自動 | 合格 | PASSED | AI | 既存テスト合格 |
| 境界値 | TRK-002 | 自動 | 合格 | PASSED | AI | NULL/既定値適用確認 |
| 境界値 | TRK-003 | 自動 | 合格 | PASSED | AI | NULL/既定値適用確認 |
| 境界値 | TRK-004 | 自動 | 合格 | PASSED | AI | NULL/既定値適用確認 |
| 境界値 | TRK-005 | 自動 | 合格 | PASSED | AI | 複合NULL/既定値適用確認 |
| 異常系 | TRK-006 | 自動 | 合格 | PASSED | AI | 型不一致ハンドリング |
| 異常系 | TRK-007 | ダミー実装 | 合格 | PASSED | AI | DB接続エラーハンドリング（Mock） |
| 統合 | TRK-008 | 手動/自動 | 合格 | PASSED | 手動担当 | 連続処理、メモリリーク なし |
| 回帰 | TRK-009 | 自動 | 合格 | PASSED | AI | 既存テスト全42件合格 |
...
| 集計軸 | 結果 |
|------|------|
| 実行ステータス別 | 合格 9 / 不合格 0 / 保留 0 / 未実施 0 |
| 実施方法別 | 自動 7 / 手動 1 / ダミー実装 1 |
| 分類別 | 正常系 2 / 境界値 4 / 異常系 2 / 統合 1 / 回帰 1 |

### CSV更新確認

- ✓ 実行後の `実行ステータス` と `実行者` を CSV に反映
- ✓ Markdown 正本との一致を確認
...
### ゲート条件「段階11」の満たし方
- [ ] チェック項目が改修点を網羅しているか
- [ ] 実施方法（自動/ダミー/手動）と実施手順が実現可能か
- [ ] CSV と Markdown 正本の整合が取れているか

### ゲート条件「段階13」の満たし方
- [ ] 全チェック項目が「合格」状態か
- [ ] ビルド/静的チェック成功か
- [ ] ダミー実装が妥当に実施されているか
- [ ] 3軸集計（ステータス/実施方法/分類）が記録済みか
