...
|-----|------|--------------|--------|--------|---------|---------|---------|--------------|--------|------|
| TRK-001 | 正常系 | [テストケース] | [前提] | [入力] | [期待] | 自動 | [実行コマンドや観点] | 未実施 | [担当者] | |
| TRK-002 | 境界値 | [テストケース] | [前提] | [入力] | [期待] | 手動 | [確認手順] | 未実施 | [担当者] | |
```
...
| **実施手順** | 実施方法に応じた具体手順（例: 実行コマンド、Mock 設定、手動確認観点） |
| **実行ステータス** | 未実施 / 実施中 / 合格 / 不合格 / 保留 / スキップ |
| **実行者** | 実施者（担当者名、または AI/手動担当の区分） |
...
| 実施方法: 自動（既存テスト利用可） | 
| 実施手順: `dotnet test ProcDataSyncTest.csproj --filter TRK-001` を実行しログ確認 | 
| 実行ステータス: 未実施 | 
| 実行者: AI | 
| 備考: 回帰テスト対象 |
```
...
| 実施方法: 自動（新規テスト） | 
| 実施手順: NULL入力モックを作成し対象メソッドを実行、既定値適用をAssert | 
| 実行ステータス: 未実施 | 
| 実行者: AI | 
...
| 実施方法: 自動（新規テスト） | 
| 実施手順: 複数NULLの入力セットを用意し、既定値が同時適用されることを確認 | 
| 実行ステータス: 未実施 | 
| 実行者: AI | 
| 備考: 複合条件の検証 |
```
...
| 実施方法: ダミー実装（Mock利用） | 
| 実施手順: Mock<IDataRecord> で SqlException を投出し catch/ログ出力を確認 | 
| 実行ステータス: 未実施 | 
| 実行者: AI | 
...
| 実施手順: 連続送信スクリプト実行後、処理件数・メモリ使用量・ログを照合 | 
| 実行ステータス: 未実施 | 
| 実行者: 手動担当 | 
| 備考: パフォーマンス観点の検証 |
```
...
| 実施方法: 自動 | 
| 実施手順: テストスイート全件実行後、失敗件数0と対象TRKの結果を照合 | 
| 実行ステータス: 未実施 | 
| 実行者: AI | 
| 備考: 改修による副作用がないことを確認 |
```
...
1. Markdown表のヘッダを `TRK,分類,テストケース名,前提条件,入力値,期待結果,実施方法,実施手順,実行ステータス,実行者,備考` の順でCSV化する。
2. ファイルは UTF-8 BOM で保存する（Excel で日本語が崩れにくい）。
3. ゲート判定前に CSV の実行ステータス列と Markdown の実行ステータス列を突合する。
